...
function BlockStartRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of BlockStartComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function VarSetRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of VarSetComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function TrialRoutineEachFrame() {
  return async function () {
    ...
    if (Trial_response.status === PsychoJS.Status.STARTED) {
      ...
      if (parseFloat(document.getElementById("lvl").innerHTML) >= 15) {
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function InputTimeRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of InputTimeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
